...
const addNoteAction = (note, user) => {
    return dispatch => fetch(
        `https://github-profiler-1ac46.firebaseio.com/${user}/notes/.json?format=export`, {
            method: 'PATCH',
            headers: {
        ...
        () => dispatch({type: ADD_NOTE, data: {id: nextActionId++, text: note}})
    );
};

const removeNoteAction = (note, user) => {
    return dispatch => fetch(
        `https://github-profiler-1ac46.firebaseio.com/${user}/notes/${note.id}.json?format=export`,
        {
            method: 'DELETE',
            headers: {
                'Accept': 'application/json',
